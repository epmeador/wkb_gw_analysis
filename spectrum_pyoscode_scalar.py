...
def spectrum(y_final, y, u_s, u_t, N, derivs1, scalarsys, tensorsys):
    ...
    for m in range(kinos):
        ...
        """
        Solve for scalar system using wkb.
        """
        

        print('Stats from scalarsys:')
       ## FIRST WE DEFINE A SCALAR SYSTEM THAT USES CONFORMAL TIME (THIS IS DONE BECAUSE PYOSCODE PREFERS THAT VARIABLE IT SEEMS--SMOOTHER FUNCTION TO EVALUATE)

    ## 1) DEFINE NGRID ON WHICH TO EVALUATE SCALAR POWER SPECTRUM
#        Ngrid_ode = np.linspace(N_start, Nfinal, 2309)

        Ngrid_ode = Nefolds[:countback+1].copy()[::-1]
        print("Ngrid_ode", Ngrid_ode[0], Ngrid_ode[-1])
        Nfinal_num = Ngrid_ode[-1]
        
#        N_lo = Nefolds[:countback+1].min()
#        N_hi = Nefolds[:countback+1].max()
#        Ngrid_ode = np.linspace(N_hi, N_lo, 2309)  # descending
#        Nfinal_num = Ngrid_ode[-1]
#

## 2) CALL THE SCALAR SYSTEM FUNCTION
        #WKB adjusted scalarsys
        vs_N_array, Ngrid_use, N_star, R_squared_scalar  = scalarsys(N_start, N_exit, Nfinal_num, Ngrid_ode, spec_params, spline1, spline2, k)
        

    ## 3) GENERATE A SPLINE OF PHI THAT GETS EVALUATED AT AN N OF YOUR CHOICE
        R_squared_spl = CubicSpline(Ngrid_use[::-1], R_squared_scalar[::-1])
    ...
    #Use an N_freeze value close to the end but not exactly at the end
        ...
        if not (Ngrid_use[-1] <= N_eval <= Ngrid_use[0]):
            raise RuntimeError(f"N_eval={N_eval} outside WKB grid "
                               f"[{Ngrid_use[-1]}, {Ngrid_use[0]}]")
        print("N_eval",N_eval)
        
    #Evaluate at N_eval
        R_squared_eval = float(R_squared_spl(N_eval))

        ## EVALUATE SCALAR POWER SPECTRUM
        
        #This one below is what I am trying to rewrite this as the scalar power specturm for each mode
...
#        Ngrid_ode = Nefolds[:count+1] #was the one equivalent to the numerical code
#        Nfinal_num = Ngrid_ode[-1]
...
def scalarsys(N_start, N_exit, Nfinal, Ngrid_to_use, spec_params, spline1, spline2, k):

    print('Stats from scalarsys:')
   ## FIRST WE DEFINE A SCALAR SYSTEM THAT USES CONFORMAL TIME (THIS IS DONE BECAUSE PYOSCODE PREFERS THAT VARIABLE IT SEEMS--SMOOTHER FUNCTION TO EVALUATE)
   
    def scalarsys_tau_solve(Ngrid_to_use, spec_params, splineH, splineeps, k, n_oscode, n_plot, rtol=1e-4,
        order=3,phase=True):
    ...
    # 3) Build omega^2(tau) = k^2 - z''/z

        N_of_tau = tau_to_N(tau)
        a_tau = spec_params.a_init * np.exp(-N_of_tau)
        
#Need to not use pygsl to get eps_tau was having issues before at eps_tau[-1] since tau[-1]=0 but fixed it now
        N_inc   = Ngrid[::-1]
        H_inc   = HN[::-1]
        eps_inc = epsN[::-1]

        H_of_N   = PchipInterpolator(N_inc, H_inc)      # or CubicSpline
...
#        eps_tau = np.array([splineeps.eval(float(N)) for N in N_of_tau])
        ...
        print("eps_tau[0] and eps_tau[-2]", eps_tau[0], eps_tau[-2])
        print("N_of_tau[0] and N_of_tau[-1]", N_of_tau[0], N_of_tau[-1])
        print("Ngrid[0] and Ngrid[-1]", Ngrid[0], Ngrid[-1])
        
        #Build z''/z
        
        aH_tau = a_tau * H_tau
        a2H2_tau = aH_tau * aH_tau
        app_over_a = a2H2_tau * (2.0 - eps_tau)
        # a''/a

#        eps_floor = 1e-30
#        L_tau = np.log(np.maximum(eps_tau, eps_floor))
        L_tau = np.log(eps_tau)
        L_spl = PchipInterpolator(tau, L_tau)
        #cna smooth if need be
#        L_spl = UnivariateSpline(tau_use, np.log(eps_use), s=1e-4)  # tune s

        L1 = L_spl.derivative(1)(tau)
        L2 = L_spl.derivative(2)(tau)

        zpp_over_z = app_over_a + 0.5*L2 + aH_tau*L1 + 0.25*(L1*L1)
        w2_tau_scalar = k*k - zpp_over_z
        
        print("w2_scalar(tau[0]) =", w2_tau_scalar[0])
        print("w2_scalar(tau[-1]) =", w2_tau_scalar[-1])

    # 4) Find turning point tau_star (where w2 crosses 0)
            #Try this way for speed enhancement, but it should be doing the same thing
            # bracket (vectorized)
        s = np.sign(w2_tau_scalar)
        ...
        w2_interp_scalar = PchipInterpolator(tau, w2_tau_scalar)
        tau_star = brentq(w2_interp_scalar, tau_L, tau_R)

        print("w^2(tau_star) =", w2_interp_scalar(tau_star))
    ...
    # 5) Prepare pyoscode inputs on ascending grid. Pyoscode requires t_eval to be in ascending order
        ts = tau  # ascending
        ws = np.emath.sqrt(w2_tau_scalar.astype(np.complex128))  # complex allowed
    ...
    # 6) BD initial conditions at earliest time (most negative tau)
        ...
        tau_i = tau[0]
        if phase:
            v0 = (1.0 / np.emath.sqrt(2.0 * k)) * np.exp(1j * k * tau_i)
        else:
            v0 = 1.0 / np.emath.sqrt(2.0 * k)
    ...
    # 7) Solve with pyoscode
        ...
        v_eval = np.array(sol["x_eval"]) #gives u(tau)

        return t_eval, v_eval, tau_star, tau_to_N, Ngrid, N_star, eps_of_N
        
        
    ## RUN THE SCALAR SYS FOR WKB BASED SOLUTION IN CONFORMAL TIME
    tau_plot, v_plot, tau_star, tau_to_N_spline, Ngrid, N_star, eps_of_N = scalarsys_tau_solve(
    ...
    eps_of_tau_plot = eps_of_N(N_of_tauplot)
    z_of_tauplot = a_of_tauplot * np.sqrt(2*eps_of_tau_plot)
    
    
    fig, axes = plt.subplots(2, 1, figsize=(10, 10), sharex=True)

    # Top panel: v(tau)
    axes[0].plot(tau_plot, np.real(v_plot), label="Re(v)")
    axes[0].plot(tau_plot, np.imag(v_plot), "--", label="Im(v)")
    axes[0].plot(tau_plot, np.abs(v_plot), label="|v|", color="k", linestyle="--")
    axes[0].axvline(0, color="gray", linestyle=":")
    axes[0].grid(True, alpha=0.3)
    axes[0].set_ylabel(r"$v_k$")
    axes[0].set_ylim(-1e32, 1e32)
    axes[0].set_title(r"Mode Function $v_k(\tau)$")
    axes[0].legend()
    
    
    # Bottom panel: R = v/z
    R_tau = v_plot / z_of_tauplot

    axes[1].plot(tau_plot, np.real(R_tau), label="Re(v/z)")
    axes[1].plot(tau_plot, np.imag(R_tau), "--", label="Im(v/z)")
    axes[1].plot(tau_plot, np.abs(R_tau), label="|v/z|", color="k", linestyle="--")
    axes[1].axvline(0, color="gray", linestyle=":")
    axes[1].grid(True, alpha=0.3)
    axes[1].set_xlabel(r"$\tau$")
    axes[1].set_ylabel(r"$R = v_k/z$")
    axes[1].set_title(r"Physical Mode $R(\tau)$")
    axes[1].legend()
...
#    
    # z(N) from N(tau) and z(tau)
    spline_z_N = PchipInterpolator(N_of_tauplot[::-1], z_of_tauplot[::-1])
    ...
    v_re_spline = PchipInterpolator(tau_plot, np.real(v_plot))
    v_im_spline = PchipInterpolator(tau_plot, np.imag(v_plot))

    # To get u(N) I take Re(u(tau(N))) + Im(u(tau(N)))
    vs_N = v_re_spline(tau_on_Ngrid) + 1j*v_im_spline(tau_on_Ngrid)
...
#    #Now it is time to save what we need to save here for our mode and apply it to the tensor power spectrum. We get the real and imaginary parts and square them
    ...
    v_squared = (vs_N_real*vs_N_real) + (vs_N_imag*vs_N_imag)
    R_squared = v_squared/(z_N*z_N)
        
    return vs_N, Ngrid, N_star, R_squared
