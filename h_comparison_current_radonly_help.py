...
#Test of Functiion
x_range = radiation_wkb_approximation(k=1)
...
# Define Q(z) and its square root
def Q(z):
    return z**2
...
# Numerical integration of S0(z)
z_values = np.linspace(0, 10, 100)
S0_numerical = np.array([quad(sqrt_Q, 0, z)[0] for z in z_values])

# Analytical solution for S0(z)
S0_analytical = z_values**3 / 3

# Plot the results
plt.figure(figsize=(8, 6))
plt.plot(z_values, S0_numerical, label="Numerical $S_0(z)$", linestyle="--", color="blue")
plt.plot(z_values, S0_analytical, label="Analytical $S_0(z) = z^3/3$", linestyle="-", color="red")
plt.xlabel("$z$", fontsize=14)
plt.ylabel("$S_0(z)$", fontsize=14)
plt.title("Comparison of Numerical and Analytical $S_0(z)$", fontsize=16)
plt.legend(fontsize=12)
plt.grid()
plt.show()


# ### Modify the WKB Analysis

# In[53]:


def plot_modified_wkb():
    # Modified parameters
    epsilon = 1
    x0 = 0
    constant = 1
...
#     analytical_h_x_nonzero = analytical_h_x[non_zero_mask]
    ...
    # Define x_array and corresponding w_array
    #x_array = np.linspace(1e-3, 10, 1000)  # Avoid x=0 to prevent divide-by-zero issues
    x_array = x_range
    non_zero_mask = x_array != 0
    x_array = x_array[non_zero_mask]
    w_array = x0 - x_array  # w = x0 - x

    # Define Q(w) function: constant -1
    Q_func = lambda w: -1  # Q(w) is constant
    ...
    analytical_solution = constant * spherical_jn(0, x_array)

    # Plot y/x vs analytical solution for comparison
    plt.figure(figsize=(10, 6))

    # Define colors for the WKB and analytical solution
    wkb_color = '#DB7093'  # Vibrant rose for WKB approximation
    analytical_color = '#4682B4'  # Soft teal for the analytical solution

    # Background shading for regions
    plt.axvspan(min(w_array), 0, color='#DDA0DD', alpha=0.2, label='Region 3 (w < 0)')
    plt.axvspan(0, max(w_array), color='#FFB6C1', alpha=0.2, label='Region 1 (w > 0)')

    # Plot the WKB Approximation and analytical solution
    plt.plot(w_array, y_general / x_array, label=r'$y/x$ (WKB Approximation)', color=wkb_color, linewidth=1.5)
    plt.plot(w_array, analytical_solution, label=r'$Analytical Solution$ (WKB Approximation)', color=analytical_color, linewidth=1.5)

    # Titles and labels
    plt.title('Rad Only: WKB Approximation vs Analytical Solution (Modified Case)', fontsize=16, weight='bold', color='#333333')
    plt.xlabel('$w$', fontsize=14, color='#333333')
    plt.ylabel(r'$y/x$', fontsize=14, color='#333333')
    plt.ylim(-1,1)
